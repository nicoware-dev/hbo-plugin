...
Work out of the box with bundled JSON demo data. No external credentials required for first run.

### O5 — Optional real integrations

Document and bundle the Composio CLI bridge as the path to Gmail, Slack, CRM, and other apps — without blocking the demo.

### O6 — Public documentation
...
| Consultants | Fast demo for AI operations pitches |

Ideal experience:

```text
User pastes install prompt → Hermes installs everything → Dashboard opens → Demo workflow runs → User approves an action
```
...
- A generic commerce ops demo workspace
...
- Production CRM sync (demo JSON for MVP)

## Success criteria

| Criteria | How to verify |
|----------|---------------|
| Install in one prompt | [Demo prompt](./demo-prompt) completes without manual CLI |
...
- Plugin + 12 tools, demo state, dashboard UI — **done**
- Profile distributions + 5 skills — **done**
...
- [Business Ops Demo](./business-ops-demo) — demo data reference
