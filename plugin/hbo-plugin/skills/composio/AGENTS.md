...
> **Platform install:** See [rules/platform-install.md](rules/platform-install.md) for Windows (WSL), Linux, and macOS. On Windows, do not use `curl | bash` in PowerShell/Git Bash or `npm install -g @composio/cli` (404).
...
**Windows (PowerShell, after WSL login):** run `.\scripts\setup-composio-windows.ps1` once, then `composio whoami` in a new PowerShell window.

**HBO Plugin:** See [rules/hbo-bridge.md](rules/hbo-bridge.md) for approvals, normalization, and `local-demo` fallback.
