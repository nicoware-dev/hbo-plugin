...
> **Windows:** Native PowerShell, CMD, and Git Bash are **not supported** for install. Use **WSL Ubuntu** for install/login. From PowerShell after login: run `.\scripts\setup-composio-windows.ps1`, then `composio` in a new window — **not** plain `wsl composio`. Do **not** use `npm install -g @composio/cli` (404 on npm).
>
> Full platform guide: [Platform Install](./platform-install.md)
...
```bash
# Install (run inside WSL on Windows, not in Git Bash)
...
composio upgrade     # if errors or new project
```

**Windows (from PowerShell, after WSL install):**

```powershell
wsl composio --version
wsl composio login
wsl composio whoami
```
