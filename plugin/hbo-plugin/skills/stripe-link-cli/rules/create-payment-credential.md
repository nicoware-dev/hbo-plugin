---
title: Create Payment Credential
impact: HIGH
description: Official Link CLI flow — auth, merchant eval, spend request, payment
tags: [stripe, link-cli, payments, card, spt, mpp]
...
Replace `<your-agent-name>` with the name of your agent or application (for example, `"Personal Assistant"`, `"Shopping Bot"`, `"HBO Sales Ops Agent"`). This name appears in the user's Link app when they approve the connection. Use a clear, unique, identifiable name.
